...
def pil_to_emnist_mlp_vector(pil_img, invert=True, do_transpose=False):
    """
    Input: PIL image (your 280x280 canvas or a 28x28 EMNIST sample)
    Output: (1, 784) float32 0..1 for MLP

    Steps:
    - grayscale
    - optional invert (Canvas needs invert=True; EMNIST samples invert=False)
    - bbox crop + margin
    - resize content to fit 20x20 (keep aspect ratio)
    - pad to 28x28
    - center-of-mass shift
    - optional transpose (ONLY if you need it; for GUI drawings usually False)
    - flatten to (1,784)
    """
    # 1) grayscale
    if pil_img.mode != "L":
    ...
    # 6) resize to fit 20x20 (keep aspect ratio)
    w, h = crop_img.size
    if w > h:
        new_w = 20
    ...
    # 9) optional transpose
    if do_transpose:
        x28 = x28.T

    # 10) flatten
    x_vec = x28.reshape(1, 784).astype(np.float32)
    return x_vec
