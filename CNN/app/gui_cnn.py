...
from .preprocessing_cnn import pil_to_emnist_cnn_tensor
from .inference_cnn import CNNDigitClassifier
...
class CNNTestApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("CNN Test – EMNIST")
        ...
        self.predict_btn = ttk.Button(btns, text="Predict", command=self.predict)
        self.predict_btn.grid(row=0, column=0, sticky="ew")
        ...
        self.model_path = os.path.join(project_root, "Test", "CNN", "models", "emnist_cnn_best.keras")

        self.classifier = CNNDigitClassifier(self.model_path)
        try:
    ...
    def predict(self):
        ...
        # IMPORTANT: invert=True for Canvas drawings
        x = pil_to_emnist_cnn_tensor(
            self.pil_img,
            invert=True,      # <-- IMPORTANT for GUI drawings
            do_transpose=False
        )
